...
public class CommandTeams extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] split) {
        ...
        if (split[0].equals("setVariable")) {
            ...
            if (TeamsManager.getInstance().currentGametype.setVariable(split[1], split[2]))
                ...
                sender.sendChatToPlayer(ChatMessageComponent.createFromText("Variable " + split[1] + " did not exist in gametype " + TeamsManager.getInstance().currentGametype.shortName));
            return;
        }
    ...
    public int compareTo(Object o) {
        if (o instanceof CommandTeams) {
            if (this.getCommandName().equalsIgnoreCase(((CommandTeams) o).getCommandName())) {
                return 0;
            }
        }
        return -1;
